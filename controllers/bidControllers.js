...
const createBid = expressAsyncHandler(async (req, res) => {
  try {
    const { key, ...bidData } = req.body;

    if (!key) {
      return res
        .status(400)
        .json({ success: false, message: "Bid key is required." });
    }

    // Check for duplicate bid using the key
    const existingBid = await Bid.findOne({ key });
    if (existingBid) {
      return res.status(409).json({
        success: false,
        message: "A bid with this key already exists.",
      });
    }
